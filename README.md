...
### Plotting & Analysis (`myace.plotting`)

This module provides high-level functions to visualize the performance of a trained potential against reference DFT data.

> **Note:** Plotting functionality requires the `plotly` library. You can install it with the core package by running:
> `pip install .[plotting]` or `rye sync --features plotting`

- **`myace.plotting.calculate_fit_metrics(df)`**: Takes a DataFrame from `pacemaker`'s prediction output and calculates key error metrics for energy and forces.
- **`myace.plotting.plot_fit_analysis(df_metrics, ...)`**: Takes the output from the function above and generates a comprehensive, interactive 3-panel HTML plot.

**Example: Visualize Pacemaker Training Results**

``````
from myace import io, plotting

# 1. Load the prediction data generated by pacemaker
#    (e.g., pacemaker --predict ... --save-prediction train_pred.pckl.gzip)
df_pred = io.read("train_pred.pckl.gzip")

# 2. Calculate all necessary error metrics
df_with_metrics = plotting.calculate_fit_metrics(df_pred)

# 3. Generate and save the plot
plotting.plot_fit_analysis(df_with_metrics, output_filename="analysis.html")

print("Analysis plot has been saved to my_fit_analysis.html")
``````
